...
export default function LessonPage({
  ...
  const components = {
    // Handle images with our custom component
    img: ({ src, alt, ...props }: { src?: string; alt?: string }) => {
      if (!src) return null;

      // Check if this is a Supabase storage path or local path
    ...
    // Handle custom Video component from markdown
  ...
  return (
    <Layout course>
      <Head>
        ...
        <meta name="description" content={`Learn ${lesson.title} in the ${course.title} course.`} />
      </Head>
...
export const getServerSideProps: GetServerSideProps<LessonPageProps> = async (
  ...
  const { category, course: courseSlug, lesson: lessonSlug } = context.params as {
    category: string;
    course: string;
    lesson: string;
  };
  ...
  if (!course.is_free) {
    ...
    if (!user) {
      return {
        redirect: {
          destination: `/login?redirect=/courses/${category}/${courseSlug}/${lessonSlug}`,
          permanent: false,
        },
      };
    }
